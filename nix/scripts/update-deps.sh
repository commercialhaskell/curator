...
lts_result=$(mktemp)
cabal_result=$(mktemp)
hackage_result=$(mktemp)
trap 'rm -f "$lts_result" "$cabal_result" "$hackage_result"' EXIT
...
#
# Step 2: Fetch LTS and Cabal info, and Hackage tarball, in parallel
#
echo ""
echo "=== Fetching Hackage tarball and LTS and Cabal versions (parallel) ==="
...
wait "$hackage_pid" || { echo "Cabal update failed"; exit 1; }
